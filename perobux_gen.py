...
channels = []
with open("perobux_gen_channels.json", "r") as f:
...
async def init_peros(ctx):
	...
	msgcount = 0
	""" For each channel in perobux_channels, look for poipero reaction and adjust entry for uid, chid """
	for chid in channels:
		channel = bot.get_channel(chid)
		msgcount += await compute_perobux(channel)
...
async def compute_perobux(channel):
	""" Get history in 200 message batches """
	oldest_message = None
	ch = channel.id
	bot.buxman.set_all_perobux(ch, 0)
	msgcount = 0
	while True:
		...
		for msg in msgs:
			...
			if peros:
				bot.buxman.adjust_perobux(uid, ch, peros[0].count)
